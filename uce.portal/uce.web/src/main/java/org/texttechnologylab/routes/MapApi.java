...
public class MapApi {
    ...
    public Route getLinkedOccurrences = ((request, response) -> {
        ...
        try {
            var gridSize = 0.5 / zoom;
            if(zoom <= 13) return gson.toJson(mapService.getTimelineMapClusters(minLng, minLat, maxLng, maxLat, gridSize, fromDate, toDate, corpusId));
            else return gson.toJson(mapService.getGeoNameTimelineLinks(minLng, minLat, maxLng, maxLat, fromDate, toDate, corpusId));
        } catch (Exception ex) {
