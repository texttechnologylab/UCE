class DrawflowJS {
    ...
    init(nodeDto) {
        ...
        this.nodes = [];
        this.placeNodeDto(nodeDto, 1, 1); // root starts at (0,0)
        activatePopovers();
    }
    ...
    addLabelNode(labelText, depth, row) {
        ...
        const html = `
        <div>
            <div class="p-2 color-secondary large-font">
                ${labelText}
            </div>
        </div>
    `;
    ...
    placeNodeDto(nodeDto, depth = 0, row = 0) {
        ...
        if (Array.isArray(nodeDto.fromNodes)) {
            nodeDto.fromNodes.forEach((fromNodeDto, i) => {
                ...
                const labelNodeId = this.addLabelNode(fromNodeDto.link.type, depth - 1, currentRow - 1);
        ...
        if (Array.isArray(nodeDto.toNodes)) {
            nodeDto.toNodes.forEach((toNodeDto, i) => {
                ...
                const labelNodeId = this.addLabelNode(toNodeDto.link.type, depth + 1, currentRow - 1);
