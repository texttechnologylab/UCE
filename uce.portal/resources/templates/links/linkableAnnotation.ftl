<div class="p-2">
    ...
    <div class="flexed mt-2 justify-content-between">
        ...
        <a class="w-rounded-btn m-0 w-100 rounded-0 pb-1 pt-1 open-document"
           data-id="${anno.getDocumentId()?string?replace('.', '')?replace(',', '')}">
        ...
        <#if anno.getPage()?has_content>
            ...
            <#if sourceanno?? && sourceanno?has_content>
                <#assign fromCoveredText = sourceanno.coveredText>
            <#else>
                <#assign fromCoveredText = "">
            </#if>
            <a class="w-rounded-btn m-0 w-100 rounded-0 pb-1 pt-1"
               onclick="openInExpandedTextView(
                       ...
                       ['${anno.getCoveredText()}', '${fromCoveredText}'])">
